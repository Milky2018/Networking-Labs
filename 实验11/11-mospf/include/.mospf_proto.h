...
struct mospf_hdr_hello {
	struct mospf_hdr hdr;
	struct mospf_hello body;
}__attribute__((packed));
#define MOSPF_HDR_HELLO_SIZE sizeof(struct mospf_hdr_hello)
...
struct mospf_hdr_lsu {
	struct mospf_hdr hdr;
	struct mospf_lsu body;
}__attribute__((packed));
#define MOSPF_HDR_LSU_SIZE sizeof(struct mospf_hdr_lsu)
...
static inline struct mospf_lsa *get_mospf_lsa(struct mospf_hdr_lsu *lsu) 
{
	return (struct mospf_lsa *)((char *)lsu + MOSPF_HDR_LSU_SIZE);
}

static void mospf_init_hdr(struct mospf_hdr *mospf, u8 type, u16 len, u32 rid, u32 aid);
static void mospf_init_hello(struct mospf_hello *hello, u32 mask);
static void mospf_init_lsu(struct mospf_lsu *lsu, u32 nadv);
