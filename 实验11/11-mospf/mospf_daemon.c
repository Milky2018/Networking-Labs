...
void *sweep_database_thread(void *param)
{
	while (1)
	{
		...
		list_for_each_entry_safe(entry, q, &mospf_db, list) if (time(NULL) - entry->timer > MOSPF_DATABASE_TIMEOUT)
		{
...
void update_path(int node_pos)
{
	for (int ite = 0; ite < node_pos; ite++)
	{
		...
		if (min_pos)
		{
			...
			for (int i = 1; i < node_pos + 1; i++)
			{
				if (visited[i] == 0 && map[min_pos][i] > 0 && (map[0][i] < 0 || (map[0][min_pos] + map[min_pos][i] < map[0][i])))
				{
...
{
	rt_entry_t *rt_tmp, *q;
	list_for_each_entry_safe(rt_tmp, q, &rtable, list) if (rt_tmp->gw != 0)
...
void handle_send_lsu()
{
		...
		list_for_each_entry(tmp_nbr, &tmp_iface->nbr_list, list)
	{
		...
		mospf_init_hdr(send_mospf, MOSPF_TYPE_LSU, MOSPF_HDR_SIZE + MOSPF_LSU_SIZE + MOSPF_LSA_SIZE * ttl_nbr, instance->router_id, instance->area_id);
		send_mospf->checksum = mospf_checksum(send_mospf);
...
void *checking_nbr_thread(void *param)
{
	while (1)
	{
		...
		list_for_each_entry(tmp_iface, &instance->iface_list, list)
			list_for_each_entry_safe(tmp_nbr, q, &tmp_iface->nbr_list, list) if (time_usage - tmp_nbr->alive >= 20)
		{
...
void handle_mospf_hello(iface_info_t *iface, const char *packet, int len)
{
	...
	if (!list_empty(&iface->nbr_list))
		list_for_each_entry(tmp, &iface->nbr_list, list) if (tmp->nbr_id == ntohl(mospf->rid))
		{
