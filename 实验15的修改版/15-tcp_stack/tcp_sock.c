...
struct tcp_sock *alloc_tcp_sock()
{
	...
	tsk->wait_write = alloc_wait_struct();
...
void free_tcp_sock(struct tcp_sock *tsk)
{
	...
	if (tsk == 0) {
		...
		free_wait_struct(tsk->wait_write);
		free(tsk);
	}
}
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	// while (packet_len > tsk->snd_wnd) {
	// log(DEBUG, "snd_wnd: %d", tsk->snd_wnd);
	while (less_than_32b(tsk->snd_wnd, packet_len)) {
		if (tsk->state != TCP_ESTABLISHED) {
			return 0;
		}
		// log(DEBUG, "sleep on wait_send");
