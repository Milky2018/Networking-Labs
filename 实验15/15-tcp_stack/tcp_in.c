...
void tcp_process(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet)
{
	...
	// log(DEBUG, IP_FMT ":%hu current state is %s.", HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport, tcp_state_str[tsk->state]);
	...
	switch (tsk->state) {
		...
		case TCP_ESTABLISHED:
			...
			} else if (flags & (TCP_PSH | TCP_ACK)) {
				tcp_update_window_safe(tsk, cb);
				tsk->snd_una = cb->ack;
				tsk->rcv_nxt = cb->seq_end;
				if (cb->pl_len > 0) {
					// log(DEBUG, "write ring buffer, len: %d", cb->pl_len);
