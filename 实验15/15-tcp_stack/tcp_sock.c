...
{
	// log(DEBUG, "in funtion read. len: %d", len);
	// log(DEBUG, "rbuf used: %d", ring_buffer_used(tsk->rcv_buf));
	...
	// log(DEBUG, "out funtion read. len: %d", read_len);
	return read_len;
}

int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	// log(DEBUG, "in funtion write. len: %d", len);
	while (tsk->snd_wnd == 0) {
	...
	// log(DEBUG, "out funtion write. len: %d", rest_len);
